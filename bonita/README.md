...
It is very likely that `bonita` does not support the currently selected architecture (`arm32v5`).
...
-	Launch the new container pointing towards the copy of DB and filesystem
	...
	```console
	$ docker run --name=bonita_7.8.4_postgres --link mydbpostgres:postgres -e "DB_NAME=newbonitadb" -e "DB_USER=newbonitauser" -e "DB_PASS=newbonitapass" -d -p 8081:8080 arm32v5/bonita:7.8.4
	```
